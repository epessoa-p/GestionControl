<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="mb-1"><i class="bi bi-box-arrow-up text-primary me-2"></i>{{ $departure ? 'Editar salida' : 'Nueva salida' }}</h1>
            <p class="text-muted mb-0">Registra salida de mercancía del almacén</p>
        </div>
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-body p-4">
            <form action="{{ $action }}" method="POST" id="departureForm">
                @csrf
                ...
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="date" name="departure_date" class="form-control @error('departure_date') is-invalid @enderror" value="{{ old('departure_date', $departure?->departure_date?->format('Y-m-d') ?? now()->format('Y-m-d')) }}" required>
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="warehouse_id" class="form-select @error('warehouse_id') is-invalid @enderror" required>
                            ...
                            @foreach($warehouses as $w)
                                <option value="{{ $w->id }}" {{ (string)old('warehouse_id', $departure?->warehouse_id) === (string)$w->id ? 'selected' : '' }}>{{ $w->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="reason" class="form-select @error('reason') is-invalid @enderror" required>
                            @foreach(\App\Models\Departure::REASON_LABELS as $val => $label)
                                <option value="{{ $val }}" {{ old('reason', $departure?->reason ?? 'sale') === $val ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="notes" class="form-control" rows="2" placeholder="Observaciones adicionales...">{{ old('notes', $departure?->notes) }}</textarea>
                    </div>
                </div>
                        ...
                        <div class="col-md-5">
                            <select name="items[0][product_id]" class="form-select form-select-sm" required>
                                ...
                                @foreach($products as $p)
                                    <option value="{{ $p->id }}" data-cost="{{ $p->cost }}" data-stock="{{ $p->current_stock }}">{{ $p->name }} ({{ $p->sku }}) - Stock: {{ $p->current_stock }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-2">
                ...
                <div class="d-flex gap-2">
                    <button class="btn btn-primary" type="submit"><i class="bi bi-check-lg me-1"></i> Guardar salida</button>
                    <a href="{{ route('departures.index') }}" class="btn btn-light border">Cancelar</a>
                </div>
            </form>
        </div>
    </div>
